...
[.lead]
Install the Connector in AWS from BlueXP, from your cloud provider's marketplace, or by manually installing the software on your own Linux host.
...
Attach the IAM role that you previously created to the Connector EC2 instance. This step is required to provide the Connector with permissions so that you can use BlueXP services.
