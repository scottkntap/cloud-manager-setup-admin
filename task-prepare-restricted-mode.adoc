...
When you use BlueXP in restricted mode, you need to use the BlueXP instance that's running on the Connector.

You can install a Connector from the AWS Marketplace, from the Azure Marketplace, or by manually installing the Connector on your own Linux host that's running in AWS, Azure, or Google Cloud.
