...
[.lead]
You should understand what's needed to get up and running with BlueXP in restricted mode so that you can prepare your environment. The preparation involves preparing a Linux host for the Connector, preparing your networking, and setting up cloud permissions.
...
Networking::
<<Refer to the section below,Prepare your networking>>.
...
Ensure that networking connections are available from the Connector and from the host where you plan to access the BlueXP console via a web browser. 
...
==== Outbound internet for day-to-day operations
