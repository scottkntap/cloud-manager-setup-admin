...
[.lead]
For an AWS commercial region, it's best to create a Connector directly from Cloud Manager, but you can launch a Connector from the AWS Marketplace, if you prefer. For AWS Government regions, you can't deploy a Connector directly from Cloud Manager, so the next best option is to do so from the AWS Marketplace.
