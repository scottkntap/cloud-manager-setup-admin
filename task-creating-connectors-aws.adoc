...
To launch the Connector in AWS, Cloud Manager needs to authenticate with AWS by either assuming an IAM role or by using AWS access keys. With either option, an IAM policy is required.

[role="quick-margin-para"]
<<Set up AWS authentication,Follow step-by-step instructions>>.
