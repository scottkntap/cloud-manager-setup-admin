...
= Install and set up the Connector in Azure
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[.lead]
Install the Connector in Microsoft Azure and then set it up for use with BlueXP.

== Install the Connector

Install the Connector directly from the BlueXP user interface, from the Azure Marketplace, or by installing the software on your own Linux host.
...
. Set up permissions for the Connector VM:

.. In the Azure portal, go to the *Access Control (IAM)* page for the Connector VM.
.. Click *Add > Add role assignment* to assign the custom role that you created for the Connector. 
+
Be sure to assign the role to the system-assigned managed identity that you enabled for the Connector VM.
+
This step is required to provide the Connector with permissions so that you can use BlueXP services.
+
https://learn.microsoft.com/en-us/azure/role-based-access-control/role-assignments-portal#step-2-open-the-add-role-assignment-page[Microsoft Azure documentation: Assign Azure roles using the Azure portal^]
...
If you installed the Connector from the Azure Marketplace or by manually installing the software, then you'll need to set up permissions for the Connector.
