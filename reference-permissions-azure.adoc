...
| Enables the management of private endpoints. Private endpoints are used when connectivity isn't provided to outside the subnet. BlueXP creates the storage account for HA with only internal connectivity within the subnet.

| "Microsoft.NetApp/netAppAccounts/capacityPools/volumes/delete",

| Enables BlueXP to delete volumes for Azure NetApp Files.
...
| Enables BlueXP to remove resources from a resource group that belong to Cloud Volumes ONTAP in case of deployment failure or deletion.
...
| Enables you to manage tags on your Azure resources using the BlueXP Tagging service.
...
| Enables BlueXP to configure an application security group for an HA pair, which isolates the HA interconnect and cluster network NICs.
