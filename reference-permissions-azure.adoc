...
.12+| Create Cloud Volumes ONTAP and stop, start, delete, and obtain the status of the system
...
| Enable Cloud Volumes ONTAP deployment from a VHD
...
.8+| Get network information about regions, the target VNet and subnet, and add Cloud Volumes ONTAP to VNets
...
.10+| Manage Azure storage accounts and disks and attach the disks to Cloud Volumes ONTAP
...
.7+| Manage an Azure load balancer for HA pairs
...
.10+| Enable the management of private endpoints for HA pairs when connectivity isn't provided outside of the subnet (Cloud Manager creates the storage account with only internal connectivity within the subnet)
...
.6+| Enables Cloud Manager to configure an application security group for an HA pair, which isolates the HA interconnect and cluster network NICs
...
.3+| Enables Cloud Manager to read, write, and delete tags associated with Cloud Volumes ONTAP resources.
