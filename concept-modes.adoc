...
While BlueXP restricts the flow of traffic, communication, and data when using restricted mode and private mode, it's your responsibility to ensure that your environment (on premises and in the cloud) is in compliance with the required regulations. 

== Overview

BlueXP offers the following deployment modes for your account. Each mode offers different functionality in terms of installation process, deployment location, authentication method, available data and storage services, and charging methods.
...
You need to enable restricted mode when you set up BlueXP from a Connector that you installed from your cloud provider's marketplace or by using the installer from the NetApp Support Site. 
+
When you log in to BlueXP for the first time from the Connector, you'll be prompted to create an account, at which point you can enable restricted mode on the account.
+
