...
The following image depicts how BlueXP obtains permissions to perform operations in Azure. A service principal object, which is tied to one or more Azure subscriptions, represents Cloud MBlueXPre Active Directory and is assigned to a custom role that allows the required permissions.
