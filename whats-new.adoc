...
link:task-creating-connectors-aws.html#create-an-iam-policy[Here's an example that shows AWS IAM role permissions used to create a Connector].

We also created a page that provides links to each of the policies. link:reference-permissions.html[View the permissions summary for Cloud Manager].
