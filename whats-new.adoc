...
The Connector is now supported in any cloud environment that has complete isolation from the internet. You can use the BlueXP console that's running on the Connector to deploy Cloud Volumes ONTAP.

Only node-based licensing (BYOL) is supported in these environments. Capacity-based licensing is not supported.
