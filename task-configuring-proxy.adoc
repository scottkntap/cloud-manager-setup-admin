...
If Cloud Volumes ONTAP systems don't have an outbound internet connection to send AutoSupport messages, Cloud Manager automatically configures those Cloud Volumes ONTAP systems to use a proxy server that's included with the Connector. link:reference-networking-cloud-manager.html#proxy-server-for-autosupport-messages[Learn more about using a proxy server to send AutoSupport messages].
