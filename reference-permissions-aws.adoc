...
If you create a Connector in a standard AWS region directly from BlueXP, BlueXP automatically applies a policy to the Connector. You don't need to do anything in this case.
...
Select your region to view the policy:

// Start snippet: collapsible block (open on page load)
.Standard regions
[%collapsible]
====
For standard regions, the permissions are spread across _two_ policies. Two policies are required due to a maximum character size limit for managed policies in AWS.
